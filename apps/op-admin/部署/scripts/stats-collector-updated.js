...
class StatsCollector {
    ...
    async calculateBasicStats(dateStr, channelCode = '', gameCode = '') {
        try {
            ...
            // 如果需要按游戏过滤，直接使用PaymentRecords表的game_code字段
            if (gameCode && gameCode !== 'all') {
                ...
                const payRate = activeUsers > 0 ? (payUsers / activeUsers * 100) : 0.0;
                ...
                return this.formatStats(activeUsers, registerUsers, validRegisterUsers, characterCount, payUsers, rechargeTimes, realRechargeAmount, payRate, activeArpu, payArpu);
            } else {
                // 不需要按游戏过滤的情况
                const realPayStatsQuery = `
                    SELECT 
    ...
    formatStats(activeUsers, registerUsers, validRegisterUsers, characterCount, payUsers, rechargeTimes, realRechargeAmount, payRate, activeArpu, payArpu) {
        return {
    ...
    async calculateLtvStats(registerDate, channelCode = '', gameCode = '') {
        try {
            ...
            // Calculate LTV for each period - 更新查询以适配新表结构
            for (const days of ltvDays) {
