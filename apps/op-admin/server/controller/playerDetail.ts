...
import { sql } from '../db';
import { getAdminWithPermissions } from '../model/admin';
...
const RESOLVE_USER_BASE_FIELDS = 'id';
...
export const getPlayerRechargeRanking = async (evt: H3Event) => {
    try {
        const adminId = Number(evt.context.adminId);
        ...
        if (!admin || admin.level !== 0) {
            throw createError({ statusCode: 403, message: '仅超级管理员可查看充值排行榜' });
        }
        ...
        if (rawIdentifier !== undefined && rawIdentifier !== null && String(rawIdentifier).trim() !== '') {
            ...
            const focusUser = await resolveUserByIdentifier(identifier);
            if (!focusUser) {
