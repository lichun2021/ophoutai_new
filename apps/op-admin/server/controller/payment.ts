...
export const handleCashierPaymentNotify = async (evt: H3Event) => {
    ...
    try {
        ...
        // ========== 月卡/终身卡激活（直接写 DB，不依赖 rechargeConfig）==========
        try {
            ...
            if (cardPriceKey) {
                ...
                if (Math.abs(notifyAmount - expectedPrice) > 0.01) {
                    console.error(`[${requestId}] 月卡金额校验失败(cashier):`, { expected: expectedPrice, notify: notifyAmount });
                } else {
                    ...
                    if (existingCard.length > 0) {
                        console.warn(`[${requestId}] 月卡已激活(cashier)，跳过重复写入:`, localOrder.transaction_id);
                    } else {
                        const cfg = CARD_DB_CONFIG[cardPriceKey];
                        const now = new Date();
                        now.setTime(now.getTime() + 8 * 60 * 60 * 1000);
                        const startAt = now.toISOString().slice(0, 10);
                        ...
                        console.log(`[${requestId}] ✅ 月卡激活成功(cashier): user=${localOrder.user_id} type=${cfg.card_type} expire=${expireAt ?? '永久'}`);
                    }
                }
            }
        } catch (cardErr: any) {
            // 月卡激活失败不影响回调成功
            console.error(`[${requestId}] 月卡激活失败(cashier, 不影响回调):`, cardErr?.message);
        }
        // ========== END 月卡激活 ==========
