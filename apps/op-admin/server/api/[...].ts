...
router.get('/admin/server-status', defineEventHandler(async (event) => {
  ...
  console.log(`[服务器状态] listActive 返回 ${allServers.length} 台服务器:`, allServers.map(s => ({ name: s.name, count_online: (s as any).count_online, webhost: s.webhost })));

  const servers = allServers.filter(s => (s as any).count_online !== 0);
  console.log(`[服务器状态] 过滤后 ${servers.length} 台（count_online != 0）`);
  ...
  const results = await Promise.allSettled(servers.map(async (s) => {
    ...
    console.log(`[服务器状态] 开始查询: ${s.name} webhost=${s.webhost} worldId=${worldId}`);
